...
function ProductivityDashboard({ report, onOpen }) {
  ...
  const rows = [
    { label: "Cumplimiento PMP", value: current.pmpCompliance, previous: previous.pmpCompliance, suffix: "%", detail: `${current.producedUnits} producidas / ${current.plannedUnits} planificadas`, formula: "Unidades producidas ÷ unidades planificadas × 100", tone: "orange", tooltip: "1. Ve a Producción > Gestión de órdenes.\n2. Asegúrate de registrar la 'Fecha de entrega pactada' de tus órdenes y avanzarlas a estado 'Terminado'." },
    { label: "Nivel de avance", value: current.progressRate, previous: previous.progressRate, suffix: "%", detail: `${current.executedActivities} ejecutadas / ${current.programmedActivities} programadas`, formula: "Actividades ejecutadas ÷ actividades programadas × 100", tone: "blue", tooltip: "1. Ve a Producción > Avance de actividades.\n2. Marca como 'Terminado' o registra progreso en las actividades de tus órdenes." },
    { label: "Lead time promedio", value: current.averageLeadTime, previous: previous.averageLeadTime, suffix: " días", detail: current.leadTimeSamples ? `${current.leadTimeSamples} órdenes con fechas completas` : "Falta registrar fecha de pedido y entrega", formula: "Fecha de entrega − fecha de pedido", tone: "purple", tooltip: "1. Ve a Producción > Gestión de órdenes.\n2. Edita una orden y registra su 'Fecha de inicio planificada' y 'Fecha de entrega pactada'." },
    { label: "Cobertura de seguridad", value: current.safetyCoverage, previous: previous.safetyCoverage, suffix: "%", detail: `${current.safetyCovered} de ${current.safetyTotal} materiales sobre el mínimo`, formula: "Disponible ≥ stock de seguridad calculado", tone: "green", tooltip: "1. Ve a Inventario > Insumos y partes.\n2. Configura el 'Stock de seguridad' y registra ingresos de stock para mantener los materiales sobre ese nivel." },
    { label: "Productividad de mano de obra", value: current.laborProductivity, previous: previous.laborProductivity, suffix: " und/HH", detail: `${current.producedUnits} unidades / ${current.reportedHours} horas reportadas`, formula: "Unidades producidas ÷ horas-hombre", tone: "teal", tooltip: "1. Ve a Producción > Partes de operación.\n2. Registra las horas trabajadas en las actividades." },
    { label: "Productividad de materiales", value: current.materialProductivity, previous: previous.materialProductivity, suffix: " und/S/", detail: current.materialProductivity == null ? "Falta costo unitario de materiales e insumos" : `Costo trazado S/ ${current.materialCost}`, formula: "Unidades producidas ÷ costo de materiales", tone: "yellow", tooltip: "1. Ve a Inventario > Insumos y partes.\n2. Edita los materiales y asígnales un Costo Unitario." },
    { label: "Productividad multifactorial", value: current.multifactorProductivity, previous: previous.multifactorProductivity, suffix: "", detail: current.multifactorProductivity == null ? "Faltan valor producido y costos de factores" : `Valor producido S/ ${current.outputValue}`, formula: "Producto total ÷ factores utilizados", tone: "navy", tooltip: "1. Registra el Costo Unitario de los materiales (Inventario).\n2. Asigna un Costo por Hora a las operaciones (Recursos).\n3. Define el Valor de Salida de las órdenes terminadas." }
  ];
...
function SimulationAlerts({ notifications }) {
  ...
  return <section className="panel alert-center">
    ...
    <div className="alert-group-list">
      ...
      {groups.map((group) => <details className={`alert-group ${group.severity}`} key={`${group.severity}-${group.category}`}>
        <summary><div className="alert-symbol">{group.severity === "critical" ? "!" : "△"}</div><div><span>{group.category}</span><strong>{group.count} {group.count === 1 ? "alerta" : "alertas"} · {group.title.replace(/^CECO \d+ /, "")}</strong></div><b>{group.value}</b><em className="expand-control"><span className="closed-label">Expandir</span><span className="open-label">Contraer</span><i>⌄</i></em></summary>
