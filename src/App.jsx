  ...
  return <div className="twin-layout">
    <aside className="scenario-panel panel">
      ...
      <div className={`twin-feed ${dataReady ? "ready" : "loading"}`}>
        <span>{dataReady ? "✓" : "…"}</span>
        <div><strong>{dataReady ? "Gemelo alimentado con Supabase" : "Cargando alimentación del gemelo"}</strong><small>{dataset.flowStages.length} fases · {dataset.stageActivities.length} actividades · {dataset.orders.length} CECO · {dataset.inventory.length} materiales · {dataset.personnel.length} personas · {dataset.equipment.length} equipos</small></div>
      </div>
...
function RecordDrawer({ drawer, dataset, onClose, onSubmit, onCreateCatalog }) {
  ...
  const titles = { material: "Registrar material", movement: "Movimiento de inventario", order: "Nueva orden CECO", product: "Registrar producto", activity: "Añadir actividad", bom: "Añadir componente BOM", operation: "Registrar parte diario", personnel: "Registrar trabajador", shift: "Registrar turno", equipment: "Registrar equipo", calendar: "Configurar día laboral", assignment: "Asignar recurso", incident: "Registrar incidencia" };
