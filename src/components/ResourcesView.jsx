...
function orderOf(dataset, ceco) { return dataset.orders.find((item) => item.ceco === ceco); }
function productOf(dataset, id) { return dataset.bodyTypes.find((item) => item.id === id); }
...
export default function ResourcesView({ dataset, openDrawer, setView }) {
  ...
  return <div className="stack-lg resources-page">
    ...
    {tab === "assignments" && <section className="panel resource-section"><Header eyebrow="Carga diaria" title="Asignaciones a CECO" detail="Consulta la capacidad comprometida. Las nuevas asignaciones se registran desde el detalle de cada orden." action={<ActionButton onClick={() => setView("orders")}>Ir a órdenes</ActionButton>} /><div className="assignment-list">{dataset.assignments.map((item) => { const order = orderOf(dataset, item.ceco); return <article key={item.id}><time>{item.assignedDate}</time><div><strong>{personOf(dataset, item.personnelId)?.name}</strong><span>CECO {item.ceco} · {productOf(dataset, order?.bodyTypeId)?.name}</span><small>{activityOf(dataset, item.activityId)?.name}</small></div><p><b>{item.plannedHours} h</b><Badge value={item.status} /></p></article>; })}</div></section>}

    {tab === "calendar" && <section className="panel resource-section"><Header eyebrow="Restricciones del calendario" title="Calendario laboral" detail="La semana base trabaja de lunes a sábado; aquí solo se registran feriados, paradas o jornadas especiales." action={<ActionButton onClick={() => openDrawer({ type: "calendar" })}>+ Registrar excepción</ActionButton>} /><div className="workweek-strip"><strong>Días laborables base</strong><span>Lun</span><span>Mar</span><span>Mié</span><span>Jue</span><span>Vie</span><span>Sáb</span><small>Domingo no laborable</small></div><div className="calendar-grid">{[...calendarExceptions].sort((a, b) => a.date.localeCompare(b.date)).map((day) => <article key={day.id} className={day.dayType}><time><b>{new Date(`${day.date}T12:00:00`).toLocaleDateString("es-PE", { day: "2-digit" })}</b><span>{new Date(`${day.date}T12:00:00`).toLocaleDateString("es-PE", { month: "short" })}</span></time><div><Badge value={day.dayType} /><strong>{calendarLabel(day)}</strong><small>{day.note || "Sin descripción"}</small></div></article>)}</div>{calendarExceptions.length === 0 && <div className="empty-state">No hay feriados ni paradas registradas.</div>}</section>}

    {tab === "incidents" && <section className="panel resource-section"><Header eyebrow="Pérdidas de capacidad" title="Incidencias operativas" detail="Consulta las pérdidas de capacidad. Los eventos asociados a una orden se registran desde su detalle." action={<ActionButton onClick={() => setView("orders")}>Ir a órdenes</ActionButton>} /><div className="incident-list">{dataset.incidents.map((item) => { const order = orderOf(dataset, item.ceco); return <article key={item.id} className={item.severity}><span className="incident-mark">!</span><div><header><Badge value={item.type} /><Badge value={item.status} /></header><strong>{item.description}</strong><p>{stageOf(dataset, item.stageId)?.name}{item.ceco ? ` · CECO ${item.ceco} · ${productOf(dataset, order?.bodyTypeId)?.name ?? "Orden"}` : ""}</p></div><aside><strong>{item.downtimeHours} h</strong><small>detención</small><time>{item.occurredAt}</time></aside></article>; })}</div></section>}
  </div>;
}
