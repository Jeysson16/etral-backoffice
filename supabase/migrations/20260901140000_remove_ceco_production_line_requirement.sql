-- La creación de CECO no depende de una línea de producción.
-- El trigger heredado hacía fallar la inserción si no llegaba una línea activa.
...
drop function if exists public.create_order_with_reservations(text, text, text, text, date);
drop function if exists public.create_order_with_reservations(text, text, text, text, date, date);
drop function if exists public.create_order_with_reservations(text, text, text, text, date, date, text);
...
begin
  ...
  values ('order-' || v_ceco, v_ceco, p_customer_name, v_customer_id, p_body_type_id, 0, null, 'orange', v_first_stage, 'En cola', 999, p_planned_start_date, p_due_date);
